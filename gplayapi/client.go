...
func NewClient(email, token string) (*GooglePlayClient, error) {
	return NewClientWithDeviceInfo(email, token, Pixel3a)
}

func NewClientWithDeviceInfo(email, token string, deviceInfo *DeviceInfo) (client *GooglePlayClient, err error) {
	authData := &AuthData{
		Email:    email,
		Locale:   "en_GB",
	}
