...
func new_aurora_OSS() (*aurora_OSS, error) {
   req, err := http.NewRequest(
      "POST", "https://auroraoss.com/api/auth", strings.NewReader(body),
   )
   if err != nil {
      return nil, err
   }
   ...
   aurora := new(aurora_OSS)
   if err := json.NewDecoder(res.Body).Decode(aurora); err != nil {
      return nil, err
   }
   return aurora, nil
}
